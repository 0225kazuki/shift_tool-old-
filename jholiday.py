 ...
 * 2010/Sep/21
 ...
 holiday_name() にキーワード引数として datetime.date のオブジェクトを取れるようにしました。
 これを指定する際は年、月、日は指定する必要がなく、指定しても無視されます。
 これにより、 jholiday モジュールを使用するスクリプトがすでに datetime.date のオブジェクトがある場合の効率が若干良くなります。

 holiday_name() は、指定した日が祝日であれば Python 2.x 以前の場合は Unicode 文字列を、 Python 3.x 以降の場合は文字列を返します。
...
また、もうひとつの方法は前述のキーワード引数 date に datetime.date のオブジェクトを渡す方法です。この場合は year, month, day は渡す必要はなく、また、渡したとしても無視されます。

holiday_name() は、その日が祝日であれば Python 2.x 系以前の場合には Unicode で祝日名を返します。
Python 3.x 系意向の場合には文字列で祝日名を返します。
"""
